...
import PlayerInfo, {deserializePlayerInfo} from "../../../gameEngine/PlayerInfo.ts"
import playGame from "../../../gameEngine/GameTest.ts"
...
function App() {
  ...
  //test the visuals
  useEffect(() => {
    playGame(3, 10, 100, setPlayerInfo);
  },[]);
  ...
  return (
    <>
      ...
      {playerInfo !== -1 && <GameComponent sendAction={(action:number)=>{
        const message = JSON.stringify({ action:action, id: clientID.current });
        socket.send(message);
